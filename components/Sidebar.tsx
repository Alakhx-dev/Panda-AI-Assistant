        ...
        <div className={`flex flex-col h-full overflow-hidden ${!isOpen && 'lg:items-center'}`}>
          ...
          <div className="p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl gradient-pink flex items-center justify-center text-white text-xl shadow-lg shrink-0">
                🐼
              </div>
              {isOpen && <span className={`font-bold text-xl tracking-tight ${config.text}`}>{t.appName}</span>}
            </div>
            {isOpen && (
              <button 
                onClick={() => setIsOpen(false)}
                className={`p-2 rounded-lg hover:bg-pink-500/10 ${config.text} lg:hidden`}
              >
          ...
          <div className="p-4">
            <button
              ...
              className={`w-full flex items-center gap-3 p-3.5 rounded-xl transition-all duration-200 border-2 ${theme === ThemeMode.DARK ? 'border-pink-500/30 hover:border-pink-500 text-pink-300' : 'border-pink-200 hover:border-pink-500 text-pink-700'} group`}
            >
          ...
          <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1.5 scrollbar-hide">
            ...
            {sessions.map(session => (
              <button
                ...
                className={`w-full text-left p-3 rounded-xl transition-all truncate text-sm flex items-center gap-3 ${
          ...
          <div className={`p-4 mt-auto border-t ${config.border} space-y-1.5`}>
            ...
            <button
              onClick={onOpenSettings}
              className={`w-full flex items-center gap-3 p-3 rounded-xl hover:bg-pink-500/10 transition-all ${config.text}`}
            >
